...
def convert_single_example(example, tokenizer, is_training):
  ...
  """将一个QA问题，很长的doc进行滑窗切分，然后根据一个example获得多个问题组合"""
  tok_to_orig_index = []
  orig_to_tok_index = []
  all_doc_tokens = [] # 用来存储子词，同时把相应的id映射关系做了对应
  features = []
  ...
  # 由于doc很长，要在doc上构造多个滑窗，来包含answer
  ...
  for (doc_span_index, doc_span) in enumerate(doc_spans):
    ...
    # 训练数据：query+滑窗doc，同时要求answer的start和end必须再doc里，否则数据无效
    if is_training:
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
  ...
  # span预测分支，一个track先把batch和seq_length进行合并
...
class FeatureWriter(object):
  ...
  def process_feature(self, feature):
    ...
    # 训练数据增加答案在文档中的start，end位置以及答案类型
    if self.is_training:
...
def compute_predictions(example):
  ...
  """根据模型预测的start和end概率分布，计算最优start和end"""
  predictions = []
  n_best_size = 10
  max_answer_length = 30
...
def compute_pred_dict(candidates_dict, dev_features, raw_results):
  """Computes official answer key from raw logits.
     通过候选答案，以及模型预测结果，以及feature得出最终处理 
  """
